...
@objc protocol StatusAlertDelegate {
...
@objc public final class StatusAlert: UIView {
    ...
    //デリゲートのインスタンスを宣言  追加
    weak var delegate: StatusAlertDelegate?
    //委譲するメソッド
    func onDismiss(){
        self.delegate?.onDismiss(sender: self)
    }
    ...
    private func createBaseLabel() -> UILabel {
        let label = StatusAlert.reusableLabelsManager.dequeueReusable()
